...
| Database | Master switch, read-only sub-permissions, and result-row limit. Disabled by default; the default limit is 25 rows. Only SELECT-style operations are permitted through a read-only JDBC connection. |
...
- `WriteFile`, `ApplyEdit`, and content-bearing `SaveFile` show the Accept/Reject diff. `CopyFile`, `MoveFile`, and `DeleteFile` are confirmed as actions, as are shell commands proposed by a backend. **Auto-accept approves all of these without asking** — see [Concurrency and limits](#concurrency-and-limits) for how long a prompt waits before it expires.
...
| Tool | Detailed description | Parameters |
|---|---|---|
| <a id="searchinfiles"></a>`SearchInFiles` | Finds literal-text or regex matches across open-project source. Results are capped at 200; the cap is not adjustable. Supplying a source `filePath` limits the search to that file's source classpath; omitting it searches every open project's Java source roots (or the project root when no Java roots are registered), independent of the focused editor. | • `query` (string, required) — literal text or regex to find<br>• `filePath` (string) — any source file in the target project; omit to search all open projects<br>• `filePattern` (string, default `*.java`) — glob filter such as `*.java` or `*.xml`<br>• `caseSensitive` (boolean, default `false`)<br>• `isRegex` (boolean, default `false`; otherwise query is literal text) |
| <a id="searchtypes"></a>`SearchTypes` | Finds Java types by name pattern. | • `name` (string, required) — type name or pattern<br>• `filePath` (string) — identifies the project<br>• `kind` (string) — restrict by type kind<br>• `includeDeps` (boolean, default `false`) — include dependency JARs |
| <a id="searchsymbols"></a>`SearchSymbols` | Finds methods, fields, and nested symbols by name. | • `name` (string, required) — symbol name or pattern<br>• `filePath` (string) — identifies the project<br>• `kind` (string) — restrict by symbol kind<br>• `includeDeps` (boolean, default `false`) — include dependency JARs |
| <a id="finddeclaration"></a>`FindDeclaration` | Resolves a symbol at a source location to its declaration. | • `line` (integer, required) — 1-based<br>• `filePath` (string) — omit to use current editor<br>• `column` (integer) — position within the line |
| <a id="findimplementations"></a>`FindImplementations` | Returns direct subtypes or implementors of a type. | • `line` (integer, required) — 1-based<br>• `filePath` (string) — omit to use current editor |
...
| <a id="getclassmembers"></a>`GetClassMembers` | Lists fields, constructors, and methods for a class. | • `className` (string) — fully qualified; needed in practice |
| <a id="gettypehierarchy"></a>`GetTypeHierarchy` | Returns supertype/subtype relationships for a class or interface. | • `className` (string) — fully qualified; needed in practice |
...
`filePath` and `line` work as a pair on these tools: supply both to target a specific symbol, or omit both to act on the cursor position. `MoveClass` is the exception — its `line` defaults to `1` rather than the cursor.

| Tool | Detailed description | Parameters |
|---|---|---|
| <a id="renamesymbol"></a>`RenameSymbol` | Renames a symbol with NetBeans reference updates. | • `newName` (string, required) — the new identifier<br>• `filePath` (string) — pairs with `line`; omit both for cursor position<br>• `line` (integer) — pairs with `filePath`; 1-based |
| <a id="moveclass"></a>`MoveClass` | Moves a Java class/package with import and reference updates. | • `targetPackage` (string, required) — e.g. `com.example.ui`<br>• `filePath` (string) — omit to use current editor<br>• `line` (integer, default `1`) — line of the class declaration |
...
| <a id="inlinevariable"></a>`InlineVariable` | Inlines a local variable at its use sites. | • `filePath` (string) — pairs with `line`; omit both for cursor position<br>• `line` (integer) — 1-based declaration or usage |
| <a id="changemethodsignature"></a>`ChangeMethodSignature` | Changes a method name, parameters, or return type and updates callers. | • `parameters` (array) — the full desired list; per item: `originalIndex` (omit = array position, `-1` = new), `name`/`type` (omit = keep), `defaultValue` (required when `originalIndex` is `-1`)<br>• `filePath` (string) — pairs with `line`; omit both for cursor position<br>• `line` (integer)<br>• `methodName` (string) — omit to keep<br>• `returnType` (string) — omit to keep<br>• `overloadMethod` (boolean) — true adds an overload instead of modifying |
...
| <a id="fiximports"></a>`FixImports` | Removes unused imports and adds resolvable missing imports. | • `filePath` (string) — omit to use current editor |
| <a id="organiseimports"></a>`OrganiseImports` | Sorts and groups imports using project formatting rules. | • `filePath` (string) — omit to use current editor |
| <a id="organisemembers"></a>`OrganiseMembers` | Reorders class members using configured member order. | • `filePath` (string) — omit to use current editor |
| <a id="reformatfile"></a>`ReformatFile` | Reformats `filePath` with project code style. | • `filePath` (string) — omit to use current editor |
...
| <a id="writefile"></a>`WriteFile` | Creates or replaces `file_path` with `content`; shows the diff review. | • `file_path` (string, required) — absolute file path<br>• `content` (string, required) — full file content |
| <a id="applyedit"></a>`ApplyEdit` | Replaces exact `old_string` with `new_string` in `file_path`; shows the diff review. | • `file_path` (string, required) — absolute file path<br>• `old_string` (string, required) — exact text to replace<br>• `new_string` (string, required) — replacement text |
| <a id="savefile"></a>`SaveFile` | Saves supplied `content` or flushes an open editor; content changes show review. Supplying `content` requires `filePath`. | • `filePath` (string) — omit for current editor<br>• `content` (string) — full replacement content; omit to flush unsaved editor |
...
| <a id="deletefile"></a>`DeleteFile` | Deletes `filePath`, closes its editor, and refreshes VCS; requires confirmation. | • `filePath` (string) — omit for current editor |
| <a id="closefile"></a>`CloseFile` | Closes `filePath`, or the current editor when omitted. | • `filePath` (string) — omit for current editor |
...
| <a id="gitstash"></a>`GitStash` | Provides the NetBeans-aware git stash operation. | • `projectPath` (string, required)<br>• `action` (string, default `push`) — push/list/pop/apply/drop<br>• `index` (integer, default `0`)<br>• `message` (string)<br>• `includeUntracked` (boolean, default `false`) |
...
`connectionName` is the display name of a connection registered in **Services > Databases**, which must already be connected. Results are capped at the configured row limit and truncation is flagged in the output.
...
| <a id="gettabledata"></a>`GetTableData` | Reads rows read-only, up to the configured row limit. | • `connectionName` (string, required)<br>• `tableName` (string, required)<br>• `limit` (integer, default the configured row limit) — capped at that limit |
| <a id="executesqlquery"></a>`ExecuteSqlQuery` | Runs a read-only SELECT; enforced by a prefix check and a read-only JDBC connection. | • `connectionName` (string, required)<br>• `sql` (string, required) — SELECT only |
...
| <a id="deleteaimessage"></a>`DeleteAiMessage` | Deletes one or several inbox messages. | • `messageId` (string) — a single ID<br>• `messageIds` (array) — bulk delete<br>• exactly one of the two is required |
