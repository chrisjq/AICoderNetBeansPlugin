...
| <a id="findfile"></a>`FindFile` | Finds files by leaf name in an accessible directory. Results are capped at 200 by default, but the header reports the true total. | • `directoryPath` (string) — absolute project or parent directory; omit to search every accessible open-project directory<br>• `pattern` (string) — optional literal text or regex matched against each file's leaf name; omit to list every file<br>• `isRegex` (boolean, default `false`; otherwise pattern is literal text)<br>• `caseSensitive` (boolean, default `false`)<br>• `maxMatches` (integer, default `200`, maximum `5000`) — `0` or omitted uses the default cap |
