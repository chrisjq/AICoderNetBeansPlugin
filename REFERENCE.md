...
| <a id="moveclass"></a>`MoveClass` | Moves a Java class/package with import and reference updates. | • `targetPackage` (string, required) — e.g. `com.example.ui`<br>• `filePath` (string, required)<br>• `line` (integer) — accepted but unused; the file's top-level class is moved |
