 ...
 * <p>
 * Every tool exposes its parameters through a per-tool {@code *ParamEnum},
 * which names the subset it accepts; the wire key itself comes from here, so
 * the twenty tools that take a file path cannot end up advertising twenty
 * slightly different spellings of it. Anything that would otherwise write a
 * property name as a string literal — {@code props.add(...)} in a schema,
 * {@code args.str(...)} in a handler — takes it from this enum instead.
 * <p>
 * Keys are camelCase without exception. {@code WriteFile} and {@code ApplyEdit}
 * previously used snake_case ({@code file_path}, {@code old_string},
 * {@code new_string}) while every other tool used camelCase, which meant an AI
 * that had learned one spelling supplied an argument the other tool silently
 * ignored. McpToolPropertyEnumTest enforces the convention.
 */
