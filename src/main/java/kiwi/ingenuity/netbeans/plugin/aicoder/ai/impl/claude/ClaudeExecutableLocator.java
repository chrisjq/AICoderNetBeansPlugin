...
public final class ClaudeExecutableLocator {
     ...
     * sandbox, absolute host paths (e.g. /usr/bin/claude) are not directly
     * executable, but the host filesystem is mounted at /run/host/, so
     * /run/host/usr/bin/claude can be executed directly without flatpak-spawn
    ...
    /**
     * Locate the claude binary. Returns the path on success, or null if not
    ...
    /**
     * Run `claude --version` with the given executable. Returns the version
