...
public abstract class AbstractFileTool implements McpToolInterface {
    ...
    public JsonObject schema(Set<McpInstructionOptionEnum> options) {
        ...
        fp.addProperty(ToolSchemaKeyEnum.DESCRIPTION.key(), "Absolute path to the file. Required — no fallback to the focused editor. Call " + McpToolEnum.GET_CURRENT_FILE.toolName() + " if you want the file the user is looking at.");
        props.add(McpToolPropertyEnum.FILE_PATH.key(), fp);
        schema.add(ToolSchemaKeyEnum.PROPERTIES.key(), props);
        JsonArray required = new JsonArray();
        required.add(McpToolPropertyEnum.FILE_PATH.key());
