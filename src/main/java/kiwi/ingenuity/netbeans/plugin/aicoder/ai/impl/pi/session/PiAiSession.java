 ...
 *
 * <p>
 * (unlike Claude, pi needs no {@code registerAlias} step. Claude's hook payload carries Claude's own CLI-internal
 * {@code session_id} — a value the plugin only learns by reading it back off the stream (see
 * {@code ClaudeStreamJsonParser#setOnFirstSessionId}) — so {@code ClaudeAiSession} maps that discovered id to itself as
 * an alias. pi's generated extension instead has {@code AICODER_CONFIG.sessionId} baked in verbatim as <em>the plugin's
 * own session UUID</em> at generation time (spec *Extension file generation and lifetime*:
 * {@code sessionId = the plugin session UUID}) — never a value discovered from pi's own RPC stream. So every hook POST
 * from the extension already carries exactly the id this class registers itself under below; the primary
 * {@link SessionRegistry#register} is resolved by {@code getId()} with no second identity to alias. The per-session
 * hook lock itself is created generically by {@code McpHookServer.registerSession}/{@code updateSessionScope} (keyed on
 * the same plugin session id) wherever a session is opened, for every AI type alike — not something this class creates
 * directly.
 */
