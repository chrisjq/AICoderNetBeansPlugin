...
public final class CodexExecutableLocator {
     ...
     * sandbox, absolute host paths (e.g. /usr/bin/codex) are not directly
     * executable, but the host filesystem is mounted at /run/host/, so
     * /run/host/usr/bin/codex can be executed directly without flatpak-spawn
    ...
    /**
     * Locate the codex binary. Returns the path on success, or null if not
    ...
    /**
     * Run `codex --version` with the given executable. Returns the version
