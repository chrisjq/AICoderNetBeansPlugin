...
public class ConversationPanel extends JScrollPane {
    ...
    private void forceScrollToBottom() {
        // Two passes, deliberately. The first validate() lays out the message that was just
        // added; only AFTER that does the scrollbar's maximum include it. Reading the maximum
        // in the same pass scrolls to where the bottom was BEFORE the new message existed,
        // landing just short of it.
        //
        // This used to be hidden: the message pane's caret was dragging the viewport down a
        // moment later and finishing the job. That side-effect has been removed (it ignored
        // the auto-scroll setting), so the scroll has to be correct on its own now.
        SwingUtilities.invokeLater(() -> {
            validate();
            SwingUtilities.invokeLater(() -> {
                validate();
    ...
    /**
     * Only auto-scroll during streaming if the user hasn't scrolled up manually.
     */
