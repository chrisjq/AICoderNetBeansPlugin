...
public final class GrokExecutableLocator {
    ...
    public static String[] platformCandidates() {
        ...
        c.add(home + "/.local/bin/grok");
        c.add(home + "/grok");
        c.add(home + "/.grok/bin/grok");
        c.add("/usr/local/bin/grok");
        c.add("/usr/bin/grok");
        if (os.contains("mac")) {
            c.add("/opt/homebrew/bin/grok");
            c.add("/usr/local/homebrew/bin/grok");
        }
        if (os.contains("win")) {
            String local = System.getenv("LOCALAPPDATA");
            if (local != null) {
                c.add(local + "\\Programs\\grok\\grok.exe");
            }
        }
