...
public final class OllamaToolHandlerFactory {

    public static Map<McpToolEnum, McpToolInterface> build(
            Supplier<AiProcessEventListener> listenerSupplier, McpHookServer server) {
        Map<McpToolEnum, McpToolInterface> map = new LinkedHashMap<>();
        map.putAll(ToolHandlerFactory.getToolHandlers(server));
        return Collections.unmodifiableMap(map);
    }

    private OllamaToolHandlerFactory() {
    }
}
