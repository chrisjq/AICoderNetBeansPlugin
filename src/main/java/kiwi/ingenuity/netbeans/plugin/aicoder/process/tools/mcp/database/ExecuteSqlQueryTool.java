...
public class ExecuteSqlQueryTool implements McpToolInterface {
    ...
    public JsonObject schema() {
        ...
        tool.addProperty(ToolSchemaKeyEnum.DESCRIPTION.key(),
                ...
                + "(currently " + PluginSettings.getDatabaseRowLimit() + ", hard ceiling 500 regardless of "
                + "configuration); if more rows exist than were returned, the output ends with a "
                + "'... (row limit N reached, results may be truncated)' note. Use ListDatabaseConnections "
                + "first to find the connectionName.");
