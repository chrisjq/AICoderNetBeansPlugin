...
/**
 * The action values {@code GitStash} accepts, each with the description shown to the caller.
 * <p>
 * These five strings were previously written out five times over: the tool description, the action property
 * description, the validation list, the "must be one of" error, and the default applied when the argument is omitted —
 * then a sixth and seventh time in {@link GitProvider#gitStash}'s switch and its default branch. Adding an action to
 * the validation list but not the switch produces the worst outcome available: the tool accepts the argument and the
 * provider silently performs a stash push instead.
 * <p>
 * Lives beside {@code GitProvider} rather than with the tool because tools depend on providers and never the reverse.
 */
public enum GitStashActionEnum {
     ...
     * that is not an action — callers must reject that rather than falling back to the default, or an unrecognised
     * action would silently stash the working tree.
     */
